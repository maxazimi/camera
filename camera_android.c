...
static const char* getCameraId(ACameraManager *cameraManager, int lens) {
    ...
    for (int i = 0; i < cameraIds->numCameras; i++) {
        ...
        // back-facing or front-facing lens
        if (facing == lens) {
            backId = id;
            break;
        }
...
static int openCamera(int cameraId, int width, int height) {
    ...
    char* id = NULL;
    if (cameraId == 0) {
        id = getCameraId(cameraManager, ACAMERA_LENS_FACING_BACK);
    } else {
        id = getCameraId(cameraManager, ACAMERA_LENS_FACING_FRONT);
    }
